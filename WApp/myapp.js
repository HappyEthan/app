...
async function getWFore () {
    let response = await fetch(WEATHERFORECASTURL);
    return await response.json();
  }
...
function generateHeader(Odata) {
  ...
  //output the value
  tempBlk.innerHTML = `<span id='temp'>${tempData}</span><span class="sup">°C</span>`;
  ...
  let rainIcon = "<img src='images/rain-48.png'>";
	let vol = Odata.rainfall.data[13].max;
...
for (let day of WFore.weatherForecast) {  //for each forecast day
    ...
    //get and output the rainfall prediction  
    let PSRimg;
    switch (day.PSR) {
      case 'High':
    ...
    //output all data and add to the containing div block
